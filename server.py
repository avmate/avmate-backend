...
collection = client.get_collection(name="avmate_regulations")

# Initialize embedding model
model = SentenceTransformer('all-MiniLM-L6-v2')

# Initialize Claude client
anthropic_client = anthropic.Anthropic(api_key=os.getenv("ANTHROPIC_API_KEY", "your-api-key-here"))
...
def search(q: Query):
    ...
    # Get relevant documents and metadatas
...
"""
    ...
    try:
        response = anthropic_client.messages.create(
            model="claude-3-opus-20240229",  # Try Opus model
            max_tokens=1000,
        ...
        example = documents[0] if documents else "No example available."
